...
class AppGUI:
    def __init__(self):
        ...
        self.translations = {
            'Français': {
                ...
                'placeholder_postal_code': 'Code Postale',
                'start': 'Démarrer',
                'stop': 'Arrêter',
                'status': 'Statut',
    ...
    def start_search(self):
        ...
        self.search_thread_1 = threading.Thread(target=self.run_search, args=('bonjoursante',))
        self.search_thread_1.daemon = True
        self.search_thread_1.start()
        self.search_thread_2 = threading.Thread(target=self.run_search, args=('rvsq',))
        ...
        with sync_playwright() as playwright:
            ...
            while self.search_running:
                try:
                    ...
                    self.log_message("[RVSQ] Creating new context...")
                    ...
                    self.log_message("[RVSQ] Accepting cookies...")
                    page.locator('#btnToutAccepter').click()
                    
                    self.log_message("[RVSQ] Filling form fields...")
                    personal_info = config['personal_info']
                    ...
                    self.log_message("[RVSQ] Checking consent checkbox...")
                    page.check('#AssureForm_CSTMT')
                    
                    self.log_message("[RVSQ] Waiting for Continue button...")
                    page.wait_for_selector('#ctl00_ContentPlaceHolderMP_myButton:not([disabled])')
                    
                    self.log_message("[RVSQ] Clicking Continue button...")
                    page.click('#ctl00_ContentPlaceHolderMP_myButton')
                    
                    self.log_message("[RVSQ] Waiting for navigation...")
                    page.wait_for_load_state('networkidle')
                    
                    self.log_message("[RVSQ] Checking if user has a family doctor...")
                    ...
                    if no_family_doctor:
                        self.log_message("[RVSQ] No family doctor detected, proceeding with appointment search...")
                        self.log_message("[RVSQ] Clicking proximity button for no family doctor case...")
                        page.click("a.h-SelectAssureBtn.ctx-changer[data-type='3']")
                    elif has_family_doctor:
                        self.log_message("[RVSQ] Family doctor detected, proceeding with appointment search...")
                        page.click("a.h-SelectAssureBtn.ctx-changer[data-type='1']")
                    else:
                        self.log_message("[RVSQ] Could not determine family doctor status")
                        return
                    
                    self.log_message("[RVSQ] Waiting for dropdown...")
                    page.wait_for_selector('#consultingReason', state='visible', timeout=60000)
                    page.wait_for_timeout(2000)
                    
                    self.log_message("[RVSQ] Selecting 'Consultation Urgente'...")
                    ...
                    if not has_family_doctor:
                        self.log_message("[RVSQ] Setting 50km radius...")
                        page.wait_for_selector('#perimeterCombo', state='visible')
                        page.wait_for_timeout(1000)
                    
                    self.log_message("[RVSQ] Clicking 'Rechercher' button...")
                    page.click('button:has-text("Rechercher")')
                    page.wait_for_load_state('networkidle')
                    
                    if has_family_doctor:
                        self.log_message("[RVSQ] Clicking GMF button...")
                        ...
                        self.log_message("[RVSQ] Clicking 'Rechercher' again...")
                        page.click('button:has-text("Rechercher")')
                        page.wait_for_load_state('networkidle')
                    ...
                    while self.search_running:  # Check if we should continue running
                        self.log_message("[RVSQ] Searching for slots...")
                        ...
                        if has_negative_indicators:
                            self.log_message("[RVSQ] No slots available")
                        elif clinic_section.is_visible():
                            self.log_message("🎉 RVSQ SLOT FOUND! 🎉")
    ...
    def run_browser_automation_bonjoursante(self, config):
        # Create screenshots directories
        for directory in ["screenshots", "error_screenshots"]:
            if not os.path.exists(directory):
                os.makedirs(directory)
        ...
        with sync_playwright() as playwright:
            ...
            while self.search_running:
                try:
                    self.log_message("[BonjourSante] Starting browser automation...")
                    ...
                    self.log_message("[BonjourSante] Creating new context...")
                    context = browser.new_context(
                        user_agent='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36'
                    )
                    context.set_default_timeout(60000) # increase from 30 sec to 60 secs for general timeout
                    ...
                    self.log_message("[BonjourSante] Navigating to form page...")
                    page.goto(
                        'https://bonjour-sante.ca/uno/clinique',
                        timeout=60000
                    )
                    
                    self.log_message("[BonjourSante] Accepting cookies...")
                    ...
                    self.log_message("[BonjourSante] Filling form fields...")
                    personal_info = config['personal_info']
                    ...
                    # Fill fields
                    self.log_message("[BonjourSante] Filling form fields part 2...")
                    ...
                    # Wait a moment for the page to load
                    page.wait_for_selector("iframe[src*='hub.bonjour-sante.ca']")
                    self.log_message("[BonjourSante] Select Options")
                    frameLocator = page.frame_locator("iframe[src*='hub.bonjour-sante.ca']")
                    ...
                    while self.search_running: 
                        page.wait_for_selector("iframe[src*='hub.bonjour-sante.ca']")
                        self.log_message("[BonjourSante] Searching for slots...")
                        frameLocator = page.frame_locator("iframe[src*='hub.bonjour-sante.ca']")
                        ...
                        if not_found or error:
                            if not_found:
                                ...
                                self.log_message("[BonjourSante] search error")
                                frameLocator.locator('a:has-text("Veuillez faire une nouvelle recherche")').click()
                            page.wait_for_selector("iframe[src*='hub.bonjour-sante.ca']")
                            frameLocator = page.frame_locator("iframe[src*='hub.bonjour-sante.ca']")
                            frameLocator.locator('button#confirm').click()
                            ...
                            self.log_message("🎉 Bonjour Sante SLOT FOUND! 🎉")
                            winsound.Beep(1000, 500)
                            winsound.Beep(2000, 500)
                            winsound.Beep(1000, 500)
                            winsound.Beep(2000, 500)
